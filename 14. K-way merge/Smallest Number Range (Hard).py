...
# currentMaxNumber - max value in current heap
# num - min value in current heap
# keep update rangeStart and rangeEnd with them..
# condition: len(minHeap) == len(lists) and len(arr) > i+1:
#   - make sure K numbers in minHeap, each one will come from each list..
#   - wont happen that two number come from same list
